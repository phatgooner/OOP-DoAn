using DoAnMonHoc_23880108.Entity;
using DoAnMonHoc_23880108.NghiepVu;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;
using System;

namespace DoAnMonHoc_23880108.Pages
{
    public class MH_SuaLoaiHangModel : PageModel
    {
		public string Chuoi { get; set; } = string.Empty;
		public string tuKhoa { get; set; } = string.Empty;

		[BindProperty]
		public string maLH { get; set; } = string.Empty;
		[BindProperty]
		public string tenLH { get; set; } = string.Empty;
		
		private IXuLyLoaiHang _xuLyLoaiHang;

		public MH_SuaLoaiHangModel() : base()
		{
			_xuLyLoaiHang = ObjectCreator.TaoDoiTuongLoaiHang();
		}

		public void OnGet()
        {
			string username = HttpContext.Session.GetString("username")!;

			if (string.IsNullOrEmpty(username))
			{
				Response.Redirect("/MH_TrangChu");
			}
			else
			{
				maLH = Request.Query["maLH"]!;
				LoaiHang? lh = _xuLyLoaiHang.DocLoaiHang(maLH);
				if (lh != null)
				{
					Chuoi = $"Sửa thông tin loại hàng mã <{lh.Ma}>";
					tenLH = lh.Ten;
				}
				else
				{
					tuKhoa = "Không tìm thấy loại hàng! ";
				}
			}
		}

		public void OnPost()
		{
			if (string.IsNullOrEmpty(tenLH))
			{
				Chuoi += "Tên loại hàng không hợp lệ! ";
			}			
			else
			{
				try
				{
					maLH = Request.Query["maLH"]!;
					LoaiHang? lh = _xuLyLoaiHang.DocLoaiHang(maLH);
					if (lh != null)
					_xuLyLoaiHang.SuaLoaiHang(maLH, lh.Ten, tenLH);
					Response.Redirect("/MH_QuanLyLoaiHang");
				}
				catch (Exception ex)
				{
					Chuoi = ex.Message;
				}
			}
		}
	}
}
